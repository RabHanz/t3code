...
| --- | --- | --- | --- | --- |
| U1 | WorkSession lifecycle: create → active → archived, and the illegal transitions | 2 | | open |
| U2 | Attach an existing thread to a WorkSession; attaching twice is idempotent; attaching a thread that already belongs elsewhere is refused | 2 | | open |
| U3 | A WorkSession survives its provider thread ending — the §29 Phase 2 exit criterion, as a unit case over the domain | 2 | | open |
| U4 | Archive/settle a WorkSession independently of thread state, and the reverse state (unarchive) exists | 2 | | open |
| U5 | Provider-session timeline: ordering, overlapping parallel sessions, a session that ends without a successor | 2 | | open |
...
| U9 | State normalization: provider and Herdr states → `FabricSessionState` (§10), including the priority order of sources | 4, 8 | | open |
...
| U12 | Synopsis staleness: age calculation, what counts as a refresh, and that a stale synopsis is labelled rather than silently served | 4 | | open |
...
| I2 | WorkSession persistence survives a server restart, read back from SQLite, not from memory | 2 | | open |
| I3 | Migration rehearsal: a `VACUUM INTO` snapshot, the migration applied, row counts before and after, and a pre-migration build still opening it (D7) | 2+ | | open |
...
| I11 | Capability advertisement: a client that does not see `fabricWorkSessions` renders the stock sidebar and calls no `fabric.*` RPC (D8) | 2 | | open |
...
| E2 | Server restart with work in flight | 2 | open |
